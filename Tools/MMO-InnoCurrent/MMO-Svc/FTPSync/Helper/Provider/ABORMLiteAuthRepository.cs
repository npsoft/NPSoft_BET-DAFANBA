...
{
    public partial class ABORMLiteAuthRepository : IUserAuthRepository, IClearable
    {
        ...
        public UserAuth CreateUserAuth(UserAuth newUser, string password)
        {
            ...
            return dbFactory.Run(db =>
            {
                ...
                newUser = db.GetById<ABUserAuth>(db.GetLastInsertId()).TranslateTo<UserAuth>();
                return newUser;
            });
        }
        ...
        {
            return dbFactory.Run(db => db.GetByIdOrDefault<ABUserAuth>(userAuthId).TranslateTo<UserAuth>());
        }
        ...
        public List<UserOAuthProvider> GetUserOAuthProviders(string userAuthId)
        {
            ...
            var kx = dbFactory.Run(db =>
                db.Select<ABUserOAuthProvider>(q => q.UserAuthId == id)).OrderBy(x => x.ModifiedDate);
            List<UserOAuthProvider> list = new List<UserOAuthProvider>();
            foreach (var e in kx)
            {
                list.Add(e.TranslateTo<UserOAuthProvider>());
            }
            return list;
        }
