...
/*-- record data into database [start] --*/
include_once('../share/config.php');
include_once('../share/mysql.php');

$field = array();
$value = array();

$array['fname'] = (!empty($_POST['fname']) ? $_POST['fname'] : '');
$array['surname'] = (!empty($_POST['surname']) ? $_POST['surname'] : '');
$array['email'] = (!empty($_POST['email']) ? $_POST['email'] : '');
$array['country'] = (!empty($_POST['country']) ? $_POST['country'] : '');
...
$query = 'INSERT INTO ' . TBL_DOWNLOAD_RECORD . ' (' . implode(',', $field) . ') VALUES (' . implode(',', $value) . ')';
mysql_execute($query);
/*-- record data into database [stop] --*/
...
mail($mailSubject,$mailBody, $mailHeaders);
