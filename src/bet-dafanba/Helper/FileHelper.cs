...
namespace SpiralEdge.Helper
{
    public class FileHelper
    {
        ...
        public static string ReadFileWithSRToLine(string path, int num)
        {
            string line = null;
            using (StreamReader sr = new StreamReader(path))
            {
        ...
        {
            using (StreamWriter sw = new StreamWriter(path, File.Exists(path), System.Text.Encoding.UTF8))
            {
                sw.Write(content);
                sw.Flush();
                sw.Close();
        ...
        public static void WriteLineFileWithSW(string path, string content)
        {
            using (StreamWriter sw = new StreamWriter(path, File.Exists(path), System.Text.Encoding.UTF8))
            {
                sw.WriteLine(content);
